...
export default {
  ...
  async created() {
    ...
    [getCateList(), getSpecsList(), getGoodsById(this.ruleForm.id)],
    );
    if (res) {
      [this.cateList, this.specsList, this.ruleForm] = res;
      this.firstCateChange(this.ruleForm.first_cateid);
      this.changeSpecs(this.ruleForm.specsid, false);
      this.goodsImgList = [{ url: this.imgUrl + this.ruleForm.img }];
      this.$refs.editor.setHtml(this.ruleForm.description);
    }
  },
  mounted() {},
  methods: {
    submitForm() {
      this.$refs.addForm.validate(async valid => {
        if (valid) {
          ...
          const res = await this.$axios.post("/api/goodsedit", fd);
          if (res.code == 200) {
            this.$success("添加成功");
