  ...
  methods: {
     submitForm() {
        this.$refs.addForm.validate(async valid => {
          ...
          const res=await this.$axios.post('/api/specsedit',this.ruleForm)
          if (res.code=200) {
            this.$success('修改成功')
      ...
      async getSpescInfo(){
          const res=await getSpecsById(this.ruleForm.id)
         
         if(res){
             this.ruleForm=res[0]
          this.attrList=res[0].attrs.map(item=>({value:item}))
         }
