<template>
  <div class=''>
      <el-button icon="el-icon-circle-plus-outline"  type="primary" @click="$router.push('/specs/add')">添加</el-button>
    ...
    <el-table-column
      ...
      <template v-slot="$scope">
       <el-tag class="bu" v-for="(i,index) in $scope.row.attrs" type="success"  :key="index">{{i}}</el-tag>
      </template>
...
export default {
  ...
  methods: {
     async getSpecsList(){
          const res=await getSpecsList({page:this.page,size:this.size})
          if(res){
            this.list=res
            console.log(res);
          }
      ...
      del(id){
        this.$myConfirm(async ()=>{
             const res=await this.$axios.post('/api/specsdelete',{id})
             if(res.code==200){
                 this.$success('删除成功')
